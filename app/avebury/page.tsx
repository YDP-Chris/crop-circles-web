...
import type { HeritageMarker } from "./AveburyMap";

const AveburyMap = dynamic(() => import("./AveburyMap"), { ssr: false });

export const revalidate = 600;

// Avebury area bbox. Captures Avebury Henge, Silbury Hill, West Kennet Long
// Barrow, the Sanctuary, and Windmill Hill.
const AVEBURY_BBOX = {
  latMin: 51.40,
  latMax: 51.46,
  lngMin: -1.90,
...
function inBbox(lat: number, lng: number): boolean {
  return (
    lat >= AVEBURY_BBOX.latMin &&
    lat <= AVEBURY_BBOX.latMax &&
    lng >= AVEBURY_BBOX.lngMin &&
    lng <= AVEBURY_BBOX.lngMax
  );
}
...
async function loadFormations(): Promise<Formation[]> {
  ...
  for (let safety = 0; safety < 50; safety++) {
    const { data, error } = await supabase
      ...
      .gte("lat", AVEBURY_BBOX.latMin)
      .lte("lat", AVEBURY_BBOX.latMax)
      .gte("lng", AVEBURY_BBOX.lngMin)
      .lte("lng", AVEBURY_BBOX.lngMax)
      .order("event_date", { ascending: false, nullsFirst: false })
      .range(start, start + PAGE - 1);

    if (error) {
      console.error("loadFormations (avebury) error", error);
      break;
    }
...
async function loadHeritage(): Promise<HeritageMarker[]> {
  ...
  const params = new URLSearchParams({
    ...
    lat: `gte.${AVEBURY_BBOX.latMin}`,
    lng: `gte.${AVEBURY_BBOX.lngMin}`,
    limit: "5000",
  });
  params.append("lat", `lte.${AVEBURY_BBOX.latMax}`);
  params.append("lng", `lte.${AVEBURY_BBOX.lngMax}`);
  ...
  if (!r.ok) {
    console.error("loadHeritage (avebury) failed", r.status, await r.text());
    return [];
  }
...
async function loadTopSites(): Promise<TopSite[]> {
  ...
  if (error) {
    console.error("loadTopSites (avebury) error", error);
    return [];
  }
...
export default async function AveburyPage() {
  ...
  return (
    <div className="findings">
      <h1>Avebury</h1>
      <p className="lead">
        The largest stone circle in Europe, ringed by a Neolithic complex unlike
        anywhere else on earth.
      </p>

      <p>
        Avebury is a UNESCO World Heritage Site and home to the largest stone
        circle in Europe. The Avebury complex &mdash; Avebury Henge, Silbury
        Hill, West Kennet Long Barrow, the Sanctuary and Windmill Hill &mdash;
        forms the densest cluster of Neolithic monuments in the United
        Kingdom. This small area is also one of the most prolific
        crop-circle locations on earth. {geotagged.length} formations from
        this corpus fall inside the bbox below ({exact.length} with exact
        coordinates), set against the named heritage sites that share the
        landscape with them.
      </p>

      <div className="map-wrap" style={{ height: 520, marginTop: 16 }}>
        <AveburyMap formations={geotagged} heritage={heritage} />
      </div>

      <h2>Top sites here</h2>
      <p>
        Heritage sites inside the Avebury bbox, ranked by how many formations
      ...
      <h2>Caveats</h2>
      <p className="small">
        Avebury-area formations in this corpus are a mix of exact-coordinate
